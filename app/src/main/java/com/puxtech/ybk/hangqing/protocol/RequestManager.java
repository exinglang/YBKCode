...
public class RequestManager {
    ...
    private void checkSessionId(MyApplication myApplication) throws JSONException, SessionErrorException {
        ...
        if(sid == null){
            JSONObject rep = null;
            try {
                rep = requestLogin(myApplication);
            } catch (IOException e) {
                e.printStackTrace();
    ...
    private JSONObject sendRequest(MyApplication myApplication, JSONObject requestData) throws JSONException, IOException, ServerException {
        HangQingLogger.i(requestData.toString(2));
        HangQingHttpSender sender = new HangQingHttpSender();
        String responseStr = null;
        String url = myApplication.getQiDongData().getHangQinFastLinkEntity().getUrl();
        responseStr = sender.requestJson(requestData.toString().getBytes(), url);
        JSONObject repJsonObject = new JSONObject(responseStr);
        HangQingLogger.i(repJsonObject.toString(2));
        return repJsonObject;
    }

    private JSONObject sendRequest(JSONObject requestData, String url) throws JSONException, IOException, ServerException {
        HangQingLogger.i(requestData.toString(2));
        HangQingHttpSender sender = new HangQingHttpSender();
        String responseStr = null;
        responseStr = sender.requestJson(requestData.toString().getBytes(), url);
     ...
     * 行情登录，指定服务器地址
     */
    public JSONObject requestLogin(String url) throws JSONException, IOException, ServerException {
        ...
        JSONObject repJsonObject = sendRequest(requestJsonObject, url);
        return repJsonObject;
    }
     ...
     * 行情登录
     */
    public JSONObject requestLogin(MyApplication myApplication) throws JSONException, IOException, ServerException {
        RequestDataCreator requestDataCreator = new RequestDataCreator();
        JSONObject requestJsonObject = requestDataCreator.createLoginRequestData();
        JSONObject repJsonObject = sendRequest(myApplication, requestJsonObject);
        return repJsonObject;
    }
    ...
    public JSONObject getMarkets(MyApplication myApplication) throws JSONException, SessionErrorException, HangQingBasicDataErrorException {
        ...
        try {
            repJsonObject = sendRequest(myApplication,requestJsonObject);
        } catch (IOException e) {
            e.printStackTrace();

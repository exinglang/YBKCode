...
//        threeRowDatas.add(threeRowData);
        ...
        try {
            viewHolder.tv_6.setText(myApplication.getHangQingData().getCommodityPriceByCommodityCode(getItem(position).getCOMMODITYID()).getZuiXinJia());
        }catch (Exception e){
            viewHolder.tv_6.setText("--");
        }
//        viewHolder.tv_6.setText(ActivityUtils.changeDouble(Math.random()+""));
        return convertView;
    }
